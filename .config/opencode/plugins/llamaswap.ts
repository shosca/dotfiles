...
export default Plugin.define({
  id: "llamaswap",
  async setup(ctx) {
    const url = (ctx.options?.url as string) || process.env.LLAMASWAP_URL || "http://localhost:8090"
    const providerID = (ctx.options?.provider as string) || process.env.LLAMASWAP_PROVIDER || "llama-swap"
